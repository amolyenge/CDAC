...
public class ProductDaoImpl implements ProductDao{
	...
	public List<Integer> findAllCategory() {
		try {
			PreparedStatement getcat=conn.prepareStatement("select distinct cid from products5 ");
	...
	public List<Product> findByCategory(int cid) {
		try {
			PreparedStatement getprod=conn.prepareStatement("select * from products5 where cid=? ");
